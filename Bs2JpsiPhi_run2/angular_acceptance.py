...
class AngularAcceptance:
    ...
    def __init__(self, omega_unbiased=None, omega_biased=None):
        """
        Initialize AngularAcceptance with omega values.
        ...
        print("AngularAcceptance initialized:")
    ...
    def eval_basis(self, cos_theta_K, cos_theta_L, phi):
        ...
        Returns:
            array of shape (n_events, 10) - the basis functions f_k for each event
        """
        ...
        ctK = cos_theta_K
        ctL = cos_theta_L
        stK = np.sqrt(np.maximum(1.0 - ctK ** 2, 1e-12))
        stL = np.sqrt(np.maximum(1.0 - ctL ** 2, 1e-12))
        sphi = np.sin(phi)
        cphi = np.cos(phi)
    ...
    def __call__(self, cos_theta_K, cos_theta_L, phi, trigger=None):
        ...
        Returns:
            Array of shape (N,) with per-candidate angular acceptance values
        """
        ...
        unnormalized_result = np.sum(f * c, axis=-1)
    ...
    def from_json(cls, json_path, year):
        """
        Load omega values from JSON file and create AngularAcceptance instance.

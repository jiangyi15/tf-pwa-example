...
mc_trigger = np.load(f"MC_time_trigger_{file_suffix}.npy")
mc_year = np.load(f"MC_time_year_{file_suffix}.npy")
n_target = len(mc_trigger)

print(f"\nTarget events (from MC_trigger): {n_target}")

# Load MC time and real data resolution
time = np.load(f"MC_time_t_smear_{file_suffix}.npy")
...
t_res = resolution[idx]

#t_smear = time + np.random.normal(size=time.shape[0]) * t_res
t_smear = time

#np.save(f"MC_time_t_smear_{file_suffix}.npy", t_smear)
np.save(f"MC_time_t_resolution_{file_suffix}.npy", t_res)

#print(f"  Saved: MC_time_t_smear_{file_suffix}.npy ({len(t_smear)} events)")
print(f"  Saved: MC_time_t_resolution_{file_suffix}.npy ({len(t_res)} events)")
...
w = np.load(f"MC_time_weight_{file_suffix}.npy")
#new_w = w * texp_weight_cut
new_w = w * cut
np.save(f"MC_time_weight_cut_{file_suffix}.npy", new_w)

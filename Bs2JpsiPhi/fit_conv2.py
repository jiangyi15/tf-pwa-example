...
config = ConfigLoader("config_conv2.yml")
for i in config.get_decay():
    for j in i:
        print(j, j.get_ls_list())
config.set_params("final_params_amp3.json")
...
fit_result.save_as("final_params_conv2.json")

trans_params = {}
with config.params_trans() as pt:
    rho0 = pt["Bs->Jpsi.phi1_g_ls_0r"]
    phi0 = pt["Bs->Jpsi.phi1_g_ls_0i"]
    rho1 = pt["Bs->Jpsi.phi1_g_ls_1r"]
    phi1 = pt["Bs->Jpsi.phi1_g_ls_1i"]
    rho2 = pt["Bs->Jpsi.phi1_g_ls_2r"]
    phi2 = pt["Bs->Jpsi.phi1_g_ls_2i"]
...
fit_result.save_as("final_params_conv2.json")
with open("trans_params_conv2.json", "w") as f:
    json.dump(save_params, f, indent=2)

exit()

config.plot_partial_wave(plot_pull=True, prefix="figure_conv2/")
config.plot_partial_wave(plot_pull=True, prefix="figure_conv2/tagp_", cut_function = lambda x: x["tag"] > 0)
config.plot_partial_wave(plot_pull=True, prefix="figure_conv2/tagm_", cut_function = lambda x: x["tag"] < 0)

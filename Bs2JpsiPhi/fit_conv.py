...
config = ConfigLoader("config_conv.yml")
...
fit_result.save_as("final_params_conv.json")
...
fit_result.save_as("final_params_conv.json")
with open("trans_params_conv.json", "w") as f:
    json.dump(save_params, f, indent=2)


config.plot_partial_wave(plot_pull=True, prefix="figure_conv/")
config.plot_partial_wave(plot_pull=True, prefix="figure_conv/tagp_", cut_function = lambda x: x["tag"] > 0)
config.plot_partial_wave(plot_pull=True, prefix="figure_conv/tagm_", cut_function = lambda x: x["tag"] < 0)
